...
class TestRNN(object):
    def __init__(self, input_dim, hidden_dim, cell_type='rnn', dtype=np.float32, seed=None):
        ...
        self.rnn = None
        if cell_type == 'rnn':
            self.rnn = VanillaRNN(input_dim, hidden_dim, init_scale=0.02, name="rnn_test")
        elif cell_type == 'lstm':
            self.rnn = LSTM(input_dim, hidden_dim, init_scale=0.02, name="rnn_test")
        ########### END ###########
...
class LanguageModelRNN(object):
    def __init__(self, word_size, word_vec_dim, hidden_dim, cell_type='rnn', dtype=np.float32, seed=None):
        ...
        self.rnn = None
        if cell_type == 'rnn':
            self.rnn = VanillaRNN(word_vec_dim, hidden_dim, init_scale=0.02, name="rnn_test")
        elif cell_type == 'lstm':
            self.rnn = LSTM(word_vec_dim, hidden_dim, init_scale=0.02, name="rnn_test")
            
        #word_embedding layer, recurrent unit, and temporal fully connected layers
